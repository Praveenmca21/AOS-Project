...
long sizeof_(char *file) {
    ...
    if (stat(file, &file_status) < 0) {
        printf("\nERROR: file size can't be determined!\n");
        exit(0);
    }
...
void write_file(int n, char *file1, int seek1, char *file2, int seek2, int mode)
{
    ...
    if(seek1){
    if(abs(seek1)>sizeof_(file1)){
        printf("\nERROR : INVALID OFFSET %d\n", seek1);
        return;
    }

    if(abs(seek2)>sizeof_(file2)){
        printf("\nERROR : INVALID OFFSET %d\n", seek2);
        return;
    }
    }
    ...
    {
        f2 = open(file2, O_WRONLY | O_APPEND);
    }
...
int main(int argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "reader") == 0)
    {
        ...
        else if (argc == 4)
        {
            ...
            read_file(atoi(argv[2]), argv[3], 0);
        }
